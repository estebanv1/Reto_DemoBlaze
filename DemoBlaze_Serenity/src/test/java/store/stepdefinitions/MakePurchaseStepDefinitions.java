...
import store.questions.TheItem;
...
//    }
    ...
    @When("he adds the items to the cart")
    public void he_adds_the_item_from_the_section_to_the_cart(DataTable dataTable) {
        List<Item> data = modelData.setItemsData(dataTable);
        for (var item : data) {
            theActorCalled("Esteban").attemptsTo(AddItem.toTheShoppingCart(
                    item));
        }
    }
    ...
    public void he_sees_the_products_in_the_shopping_cart(DataTable dataTable) {
        List<Item> data = modelData.setItemsData(dataTable);
        for (var item : data) {
            theActorCalled("Esteban").should(GivenWhenThen.seeThat(TheItem.isPresent(item.getName())));
        }
